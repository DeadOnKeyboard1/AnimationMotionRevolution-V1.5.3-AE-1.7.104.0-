...
namespace REL {
    ...
    public:
        ...
        [[nodiscard]] inline std::size_t id2offset(std::uint64_t a_id) const {
            ...
            if (it == _id2offset.end()) {
                failed = true;
            } else if (it->id != a_id || it->offset == 0) {
                failed = true;
            }
            if (failed) {
                stl::report_and_fail(
    ...
    private:
        ...
        public:
            void read(istream_t &a_in, std::uint8_t a_formatVersion) {
                ...
                if (format == 5) {
                    char name[64]{};
                    a_in.readin(name);
                    std::int64_t ptrSize{0};
                    std::int64_t addrCount{0};
                    a_in.readin(ptrSize);
                    a_in.readin(addrCount);
                    _pointerSize = static_cast<std::int32_t>(ptrSize);
                    _addressCount = static_cast<std::int32_t>(addrCount);
                } else {
                    std::int32_t nameLen{};
                    a_in.readin(nameLen);
                    a_in.ignore(nameLen);
        ...
        void load() {
            ...
            // Always resolve from SkyrimSE.exe. Mod Organizer 2 can use its own
            // working directory, so a relative Data path is not reliable there.
            const auto gameDirectory = std::filesystem::path(Module::get().filePath()).parent_path();
            if SKYRIM_REL_CONSTEXPR (Module::IsVR()) {
                const auto filename = (gameDirectory / L"Data" / L"SKSE" / L"Plugins" /
                                       stl::utf8_to_utf16(fmt::format("version-{}.csv"sv, version.string()))
                                               .value_or(L"<unknown filename>"s))
                                              .wstring();
                load_csv(filename, version, true);
            } else {
                const auto databaseName = Module::IsAE() ?
                                          fmt::format("versionlib-{}.bin"sv, version.string()) :
                                          fmt::format("version-{}.bin"sv, version.string());
                const auto filename = (gameDirectory / L"Data" / L"SKSE" / L"Plugins" /
                                       stl::utf8_to_utf16(databaseName).value_or(L"<unknown filename>"s))
                                              .wstring();
        ...
        bool unpack_file(istream_t &a_in, header_t a_header, bool a_failOnError) {
            if (a_header.format() == 5) {
                const std::size_t total_in_file = a_header.address_count() > 0 ? a_header.address_count() - 1 : 0;
                std::size_t out_idx = 0;
                for (std::size_t i = 0; i < total_in_file; ++i) {
                    std::uint32_t off32{};
                    a_in.readin(off32);
                    if (out_idx < _id2offset.size()) {
                        _id2offset[out_idx] = { static_cast<std::uint64_t>(i + 1), static_cast<std::uint64_t>(off32) };
                        ++out_idx;
                    }
                }
                while (out_idx < _id2offset.size()) {
                    _id2offset[out_idx] = { 0, 0 };
                    ++out_idx;
                }
                return true;
            }
